...
const isNullifierSpent = async (
    ...
    if (!bitset) {
        const account = await program.account.nullifierSet.fetch(
            deriveNullifierSet(program.programId, mint, chunkIndex)
        ).catch(() => null);
        if (!account) {
...
export async function rescanNotesForOwner(params: {
    ...
    const indices =
        viewKeyIndices && viewKeyIndices.length > 0
            ...
            : Array.from({ length: Math.max(0, viewKeyMaxIndex ?? VIEW_KEY_SCAN_MAX_INDEX) + 1 }, (_, index) => index);
    ...
    const viewKeyTags = await Promise.all(
        viewKeys.map((entry) => recipientTagHashFromViewKey(entry.pubkey))
    );
    onStatus?.(
    ...
    const balance = mergedNotes.filter((note) => !note.spent).reduce((sum, note) => sum + BigInt(note.amount), 0n);
