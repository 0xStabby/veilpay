...
export async function rescanIdentityRegistry(params: {
  ...
  for (const sig of orderedSignatures) {
    ...
    for (const ix of instructions) {
      ...
      const isTargetProgram = !!programId && new PublicKey(programId).equals(program.programId);
      let handled = false;
      for (const candidate of extractByteCandidates(ix.data)) {
        if (isTargetProgram) {
          const decoded = decodeInstruction(program, candidate);
          if (decoded) {
            if (decoded.name !== "registerIdentity" && decoded.name !== "register_identity") {
              continue;
            }
            seenRegisters += 1;
  ...
  if (onChainRoot.length === 32 && commitments.length > 0) {
    ...
    if (!matches) {
      ...
      onStatus?.(
        `Identity root local=${Buffer.from(localRoot).toString("hex")} onchain=${Buffer.from(onChainRoot).toString(
          "hex"
        )}`
      );
      if (decodedNewRootHex) {
