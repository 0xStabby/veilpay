...
if [[ -z "$VEILPAY_PROGRAM_ID" || -z "$VERIFIER_PROGRAM_ID" ]]; then
...
anchor build
anchor upgrade --program-id "$VEILPAY_PROGRAM_ID" -u devnet target/deploy/veilpay.so

echo "Upgrading verifier: $VERIFIER_PROGRAM_ID"
anchor upgrade --program-id "$VERIFIER_PROGRAM_ID" -u devnet target/deploy/verifier.so
