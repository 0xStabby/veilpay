...
if [[ "$SKIP_CIRCUITS" != "1" ]]; then
  bash "$ROOT_DIR/scripts/build-circuits.sh"
fi

setup_args=()
if [[ "$SKIP_ADMIN_BOOTSTRAP" == "1" ]]; then
  setup_args+=("--skip-admin-bootstrap")
fi
bash "$ROOT_DIR/scripts/setup-devnet.sh" "${setup_args[@]}"
sync_relayer_env
...
cat <<MSG
Deploy complete.
- Programs deployed, app env + IDL refreshed
- Relayer env synced at $RELAYER_ENV_FILE
MSG

if [[ "$SKIP_CIRCUITS" != "1" ]]; then
  echo "Circuits rebuilt and app/relayer artifacts refreshed."
else
  echo "Circuits rebuild skipped."
fi
